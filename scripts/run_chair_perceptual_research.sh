...
if [[ "${COMPLETED_STEPS}" == "${STOP_STEP}" ]] \
    ...
    echo "ERROR: E6 output cannot recover to requested step: ${OUTPUT_DIR}" >&2
    exit 2
fi
...
"${COMMAND[@]}"
echo "Perceptual research evidence ready at step ${STOP_STEP}: ${OUTPUT_DIR}"
