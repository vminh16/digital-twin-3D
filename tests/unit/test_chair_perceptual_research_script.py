...
SCRIPT = REPO_ROOT / "scripts" / "run_chair_perceptual_research.sh"


def test_chair_perceptual_script_is_staged_and_recoverable() -> None:
    script = SCRIPT.read_text(encoding="utf-8")

    assert script.startswith("#!/usr/bin/env bash\nset -euo pipefail\n")
    assert "E6-chair-observation-scale-perceptual-v1" in script
    assert 'STOP_STEP="${E6_STOP_STEP:-15000}"' in script
